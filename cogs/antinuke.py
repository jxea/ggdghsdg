...
def check_whitelist():
 async def predicate(ctx: commands.Context):
   ...
   async with ctx.bot.db.cursor() as cursor: 
    ...
    if check is None: 
       await ctx.reply(embed=discord.Embed(color=0x2f3136, description=f"<:stop:1034791708216533074> {ctx.author.mention}: You are not whitelisted"), mention_author=False)
       return False
...
def check_owner():
  async def predicate(ctx: commands.Context):
    ...
    if ctx.author.id != ctx.guild.owner.id:
      await ctx.reply(embed=discord.Embed(color=0x2f3136, description=f"<:stop:1034791708216533074> {ctx.author.mention}: Only the server owner can use this command"), mention_author=False)
      return False  
...
class Antinuke(commands.Cog): 
    ...
    async def whitelist(self, ctx): 
      embed = discord.Embed(color=0x2f3136, title="group command: whitelist", description="whitelist your trusted members to prevent them being detected by the antinuke")
    ...
    async def list(self, ctx): 
          ...
          async with self.bot.db.cursor() as cursor:
            ...
            for result in results:
              ...
              if l == 10:
               messages.append(mes)
               number.append(discord.Embed(color=0x2f3136, title=f"whitelisted ({len(results)})", description=messages[i]))
               i+=1
               mes = ""
               l=0
    
            messages.append(mes)
            embed = discord.Embed(color=0x2f3136, title=f"whitelisted ({len(results)})", description=messages[i])
            number.append(embed)
            if len(number) > 1:
    ...
    async def add(self, ctx: commands.Context, *, member: discord.Member=None): 
      ...
      async with ctx.bot.db.cursor() as cursor: 
        ...
        check = await cursor.fetchone()
        if check is not None: return await ctx.reply(embed=discord.Embed(color=0x2f3136, description=f"<:Emojis.warninged:1034791706551406592> {ctx.author.mention}: This user is already whitelisted"), mention_author=False)
        await cursor.execute("INSERT INTO whitelist VALUES (?,?)", (ctx.guild.id, member.id))
        await self.bot.db.commit()
        await ctx.reply(embed=discord.Embed(color=0x2f3136, description=f"<:check2:1034791707679666186> {ctx.author.mention}: Whitelisted {member.mention}"), mention_author=False)
    ...
    async def remove(self, ctx, *, member: discord.Member):
      ...
      async with ctx.bot.db.cursor() as cursor: 
        await cursor.execute("SELECT * FROM whitelist WHERE guild_id = {} AND user_id = {}".format(ctx.guild.id, member.id))
        check = await cursor.fetchone()
        if check is None: return await ctx.reply(embed=discord.Embed(color=0x2f3136, description=f"<:Emojis.warninged:1034791706551406592> {ctx.author.mention}: This user is not whitelisted"), mention_author=False)
        await cursor.execute("DELETE FROM whitelist WHERE guild_id = {} AND user_id = {}".format(ctx.guild.id, member.id))
        await self.bot.db.commit()
        await ctx.reply(embed=discord.Embed(color=0x2f3136, description=f"<:check2:1034791707679666186> {ctx.author.mention}: Removed whitelist from {member.mention}"), mention_author=False)

    @commands.group(aliases=["an"], invoke_without_command=True)
    @commands.cooldown(1, 2, commands.BucketType.user)
    @check_whitelist()
    async def antinuke(self, ctx): 
      embed = discord.Embed(color=0x2f3136, title="group command: antinuke", description="protect your server against nukes and raids")
      embed.set_thumbnail(url=self.bot.user.display_avatar.url)
    ...
    async def settings(self, ctx): 
      vanity = "<:icons_off:1035239789982654565>"
      ban = "<:icons_off:1035239789982654565>"
      kick = "<:icons_off:1035239789982654565>"
      channel = "<:icons_off:1035239789982654565>"
      roleupdate = "<:icons_off:1035239789982654565>"
      roledelete = "<:icons_off:1035239789982654565>"
      async with self.bot.db.cursor() as cursor: 
         ...
         for result in results: 
           ...
           elif result[1] == "roledelete": roledelete = "<:icons_on:1035239789324152892>"

         embed = discord.Embed(color=0x2f3136, title="antinuke settings")
    ...
    async def vanity(self, ctx: commands.Context, option=None, punishment=None):
      ...
      if option.lower() == "set":
       ...
       if not punishment.lower() in ["ban", "kick", "strip"]: return await ctx.reply(embed=discord.Embed(color=0x2f3136, description=f"<:Emojis.warninged:1034791706551406592> {ctx.author.mention}: **{punishment}** is an invalid punishment. Please choose between ban, kick or strip"), mention_author=False)
       async with self.bot.db.cursor() as cursor: 
        await cursor.execute(f"SELECT * FROM antinuke WHERE guild_id = {ctx.guild.id} AND module = '{ctx.command.name}'")
        check = await cursor.fetchone()
        if check is not None: return await ctx.reply(embed=discord.Embed(color=0x2f3136, description=f"<:Emojis.warninged:1034791706551406592> {ctx.author.mention}: Anti vanity update module is already configured"), mention_author=False)
        await cursor.execute("INSERT INTO antinuke VALUES (?,?,?)", (ctx.guild.id, ctx.command.name, punishment))
        await self.bot.db.commit()
        await ctx.reply(embed=discord.Embed(color=0x2f3136, description=f"<:check2:1034791707679666186> {ctx.author.mention}: Added anti vanity update module"), mention_author=False)
      elif option.lower() == "unset": 
       async with self.bot.db.cursor() as cursor: 
        await cursor.execute(f"SELECT * FROM antinuke WHERE guild_id = {ctx.guild.id} AND module = '{ctx.command.name}'")
        check = await cursor.fetchone()
        if check is None: return await ctx.reply(embed=discord.Embed(color=0x2f3136, description=f"<:Emojis.warninged:1034791706551406592> {ctx.author.mention}: Anti vanity update module is not configured"), mention_author=False)  
        await cursor.execute(f"DELETE FROM antinuke WHERE guild_id = {ctx.guild.id} AND module = '{ctx.command.name}'")
        await ctx.reply(embed=discord.Embed(color=0x2f3136, description=f"<:check2:1034791707679666186> {ctx.author.mention}: Removed anti vanity update module"), mention_author=False)
    ...
    async def ban(self, ctx: commands.Context, option=None, punishment=None):
      ...
      if option.lower() == "set":
       ...
       async with self.bot.db.cursor() as cursor: 
        await cursor.execute(f"SELECT * FROM antinuke WHERE guild_id = {ctx.guild.id} AND module = '{ctx.command.name}'")
        check = await cursor.fetchone()
        if check is not None: return await ctx.reply(embed=discord.Embed(color=0x2f3136, description=f"<:Emojis.warninged:1034791706551406592> {ctx.author.mention}: Anti ban module is already configured"), mention_author=False)
        await cursor.execute("INSERT INTO antinuke VALUES (?,?,?)", (ctx.guild.id, ctx.command.name, punishment))
        await self.bot.db.commit()
        await ctx.reply(embed=discord.Embed(color=0x2f3136, description=f"<:check2:1034791707679666186> {ctx.author.mention}: Added anti ban module"), mention_author=False)
      elif option.lower() == "unset": 
       async with self.bot.db.cursor() as cursor: 
        await cursor.execute(f"SELECT * FROM antinuke WHERE guild_id = {ctx.guild.id} AND module = '{ctx.command.name}'")
        check = await cursor.fetchone()
        if check is None: return await ctx.reply(embed=discord.Embed(color=0x2f3136, description=f"<:Emojis.warninged:1034791706551406592> {ctx.author.mention}: Anti ban is not configured"), mention_author=False)  
        await cursor.execute(f"DELETE FROM antinuke WHERE guild_id = {ctx.guild.id} AND module = '{ctx.command.name}'")
        await ctx.reply(embed=discord.Embed(color=0x2f3136, description=f"<:check2:1034791707679666186> {ctx.author.mention}: Removed anti vaniy module"), mention_author=False)
    ...
    async def kick(self, ctx: commands.Context, option=None, punishment=None):
      ...
      if option.lower() == "set":
       ...
       if not punishment.lower() in ["ban", "kick", "strip"]: return await ctx.reply(embed=discord.Embed(color=0x2f3136, description=f"<:Emojis.warninged:1034791706551406592> {ctx.author.mention}: **{punishment}** is an invalid punishment. Please choose between kick, kick or strip"), mention_author=False)
       async with self.bot.db.cursor() as cursor: 
        await cursor.execute(f"SELECT * FROM antinuke WHERE guild_id = {ctx.guild.id} AND module = '{ctx.command.name}'")
        check = await cursor.fetchone()
        if check is not None: return await ctx.reply(embed=discord.Embed(color=0x2f3136, description=f"<:Emojis.warninged:1034791706551406592> {ctx.author.mention}: Anti kick module is already configured"), mention_author=False)
        await cursor.execute("INSERT INTO antinuke VALUES (?,?,?)", (ctx.guild.id, ctx.command.name, punishment))
        await self.bot.db.commit()
        await ctx.reply(embed=discord.Embed(color=0x2f3136, description=f"<:check2:1034791707679666186> {ctx.author.mention}: Added anti kick module"), mention_author=False)
      elif option.lower() == "unset": 
       async with self.bot.db.cursor() as cursor: 
        await cursor.execute(f"SELECT * FROM antinuke WHERE guild_id = {ctx.guild.id} AND module = '{ctx.command.name}'")
        check = await cursor.fetchone()
        if check is None: return await ctx.reply(embed=discord.Embed(color=0x2f3136, description=f"<:Emojis.warninged:1034791706551406592> {ctx.author.mention}: Anti kick is not configured"), mention_author=False)  
        await cursor.execute(f"DELETE FROM antinuke WHERE guild_id = {ctx.guild.id} AND module = '{ctx.command.name}'")
        await ctx.reply(embed=discord.Embed(color=0x2f3136, description=f"<:check2:1034791707679666186> {ctx.author.mention}: Removed anti vaniy module"), mention_author=False)
    ...
    async def channel(self, ctx: commands.Context, option=None, punishment=None):
      ...
      if option.lower() == "set":
       ...
       if not punishment.lower() in ["ban", "kick", "strip"]: return await ctx.reply(embed=discord.Embed(color=0x2f3136, description=f"<:Emojis.warninged:1034791706551406592> {ctx.author.mention}: **{punishment}** is an invalid punishment. Please choose between channel, channel or strip"), mention_author=False)
       async with self.bot.db.cursor() as cursor: 
        await cursor.execute(f"SELECT * FROM antinuke WHERE guild_id = {ctx.guild.id} AND module = '{ctx.command.name}delete'")
        check = await cursor.fetchone()
        if check is not None: return await ctx.reply(embed=discord.Embed(color=0x2f3136, description=f"<:Emojis.warninged:1034791706551406592> {ctx.author.mention}: Anti channel delete module is already configured"), mention_author=False)
        await cursor.execute("INSERT INTO antinuke VALUES (?,?,?)", (ctx.guild.id, ctx.command.name+"delete", punishment))
        await self.bot.db.commit()
        await ctx.reply(embed=discord.Embed(color=0x2f3136, description=f"<:check2:1034791707679666186> {ctx.author.mention}: Added anti channel delete module"), mention_author=False)
      elif option.lower() == "unset": 
       async with self.bot.db.cursor() as cursor: 
        await cursor.execute(f"SELECT * FROM antinuke WHERE guild_id = {ctx.guild.id} AND module = '{ctx.command.name}delete'")
        check = await cursor.fetchone()
        if check is None: return await ctx.reply(embed=discord.Embed(color=0x2f3136, description=f"<:Emojis.warninged:1034791706551406592> {ctx.author.mention}: Anti channel delete is not configured"), mention_author=False)  
        await cursor.execute(f"DELETE FROM antinuke WHERE guild_id = {ctx.guild.id} AND module = '{ctx.command.name}delete'")
        await ctx.reply(embed=discord.Embed(color=0x2f3136, description=f"<:check2:1034791707679666186> {ctx.author.mention}: Removed anti channel delete module"), mention_author=False)
    ...
    async def roledelete(self, ctx: commands.Context, option=None, punishment=None):
      ...
      if option.lower() == "set":
       ...
       if not punishment.lower() in ["ban", "kick", "strip"]: return await ctx.reply(embed=discord.Embed(color=0x2f3136, description=f"<:Emojis.warninged:1034791706551406592> {ctx.author.mention}: **{punishment}** is an invalid punishment. Please choose between role, role or strip"), mention_author=False)
       async with self.bot.db.cursor() as cursor: 
        await cursor.execute(f"SELECT * FROM antinuke WHERE guild_id = {ctx.guild.id} AND module = 'roledelete'")
        check = await cursor.fetchone()
        if check is not None: return await ctx.reply(embed=discord.Embed(color=0x2f3136, description=f"<:Emojis.warninged:1034791706551406592> {ctx.author.mention}: Anti role delete module is already configured"), mention_author=False)
        await cursor.execute("INSERT INTO antinuke VALUES (?,?,?)", (ctx.guild.id, "roledelete", punishment))
        await self.bot.db.commit()
        await ctx.reply(embed=discord.Embed(color=0x2f3136, description=f"<:check2:1034791707679666186> {ctx.author.mention}: Added anti role delete module"), mention_author=False)
      elif option.lower() == "unset": 
       async with self.bot.db.cursor() as cursor: 
        await cursor.execute(f"SELECT * FROM antinuke WHERE guild_id = {ctx.guild.id} AND module = 'roledelete'")
        check = await cursor.fetchone()
        if check is None: return await ctx.reply(embed=discord.Embed(color=0x2f3136, description=f"<:Emojis.warninged:1034791706551406592> {ctx.author.mention}: Anti role delete is not configured"), mention_author=False)  
        await cursor.execute(f"DELETE FROM antinuke WHERE guild_id = {ctx.guild.id} AND module = 'roledelete'")
        await ctx.reply(embed=discord.Embed(color=0x2f3136, description=f"<:check2:1034791707679666186> {ctx.author.mention}: Removed anti role delete module"), mention_author=False)
    ...
    async def roleupdate(self, ctx: commands.Context, option=None, punishment=None):
      ...
      if option.lower() == "set":
       ...
       async with self.bot.db.cursor() as cursor: 
        ...
        check = await cursor.fetchone()
        if check is not None: return await ctx.reply(embed=discord.Embed(color=0x2f3136, description=f"<:Emojis.warninged:1034791706551406592> {ctx.author.mention}: Anti role update module is already configured"), mention_author=False)
        await cursor.execute("INSERT INTO antinuke VALUES (?,?,?)", (ctx.guild.id, "roleupdate", punishment))
        await self.bot.db.commit()
        await ctx.reply(embed=discord.Embed(color=0x2f3136, description=f"<:check2:1034791707679666186> {ctx.author.mention}: Added anti role update module"), mention_author=False)
      elif option.lower() == "unset": 
       async with self.bot.db.cursor() as cursor: 
        await cursor.execute(f"SELECT * FROM antinuke WHERE guild_id = {ctx.guild.id} AND module = 'roleupdate'")
        check = await cursor.fetchone()
        if check is None: return await ctx.reply(embed=discord.Embed(color=0x2f3136, description=f"<:Emojis.warninged:1034791706551406592> {ctx.author.mention}: Anti role update is not configured"), mention_author=False)  
        await cursor.execute(f"DELETE FROM antinuke WHERE guild_id = {ctx.guild.id} AND module = 'roleupdate'")
        await ctx.reply(embed=discord.Embed(color=0x2f3136, description=f"<:check2:1034791707679666186> {ctx.author.mention}: Removed anti role update module"), mention_author=False)
