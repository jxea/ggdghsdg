...
class autoresponder(commands.Cog):
    ...
    @commands.command(help="set an autoresponder for the server", description="utility", usage="[subcommand] [trigger] [response]", brief="autoresponder add - add an autoresponder\nautoresponder remove - remove an autoresponder\nautoresponder list - see a list of autoresponders", aliases=["ar"])
       ...
       async with self.bot.db.cursor() as cursor: 
           ...
           if check is not None: 
            ...
            await sendmsg(self, ctx, None, discord.Embed(color=Colors.green, description=f"{Emojis.check} {ctx.author.mention}: deleted autoresponder with trigger `{trigger}`"), None, None, None) 
             ...
             await sendmsg(self, ctx, None, discord.Embed(color=Colors.yellow, description=f"{Emojis.Emojis.warninging} {ctx.author.mention}: there is no autoresponder with trigger `{trigger}`"), None, None, None) 
          ...
          async with self.bot.db.cursor() as cursor:
            ...
            for result in results:
              ...
              if l == 10:
               messages.append(mes)
               number.append(discord.Embed(color=Colors.default, title=f"autoresponders ({len(results)})", description=messages[i]))
               i+=1
               mes = ""
               l=0
    
            messages.append(mes)
            embed = discord.Embed(color=Colors.default, title=f"autoresponders ({len(results)})", description=messages[i])
            number.append(embed)
            if len(number) > 1:
