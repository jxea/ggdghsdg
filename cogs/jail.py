...
class jail(commands.Cog):
    ...
    async def setme(self, ctx: commands.Context):
     ...
     async with self.bot.db.cursor() as cursor:
      ...
      if res is not None: return await sendmsg(self, ctx, None, discord.Embed(color=Colors.yellow, description=f"{Emojis.Emojis.warning} {ctx.author.mention}: Jail is already set"), None, None, None)
      role = await ctx.guild.create_role(name="vital - jail", color=0x000000)
    ...
    async def unsetme(self, ctx: commands.Context):
     ...
     async with self.bot.db.cursor() as cursor:
      ...
      check = await cursor.fetchone()
      if check is None:
       em = discord.Embed(color=Colors.yellow, description=f"{Emojis.Emojis.warning} {ctx.author.mention}: jail module is not set")
      ...
      async def button2_callback(interaction: discord.Interaction):
       ...
       embed = discord.Embed(color=Colors.green, description=f"{Emojis.check} {ctx.author.mention}: you have changed your miind") 
       await interaction.response.edit_message(embed=embed, view=None) 
    ...
    async def jail(self, ctx: commands.Context, member: discord.Member=None, *, reason=None):
     ...
     if member.top_role >= ctx.author.top_role and ctx.author.id != ctx.guild.owner.id:
       nope = discord.Embed(color=Colors.yellow, description=f"{Emojis.Emojis.warning} {ctx.author.mention}: you can't jail {member.mention}")
     ...
     async with self.bot.db.cursor() as cursor: 
      ...
      chec = await cursor.fetchone()
      if chec is None:
        em = discord.Embed(color=Colors.yellow, description=f"{Emojis.Emojis.warning} {ctx.author.mention} use `setme` command before using jail")
        await sendmsg(self, ctx, None, em, None, None, None) 
        return
      ...
      check = await cursor.fetchone()
      if check is not None:
        em = discord.Embed(color=Colors.yellow, description=f"{Emojis.Emojis.warning} {ctx.author.mention}: {member.mention} is jailed already")
        await sendmsg(self, ctx, None, em, None, None, None) 
        return
    ...
    async def unjail(self, ctx, *, member: discord.Member=None):
     ...
     if member.top_role >= ctx.author.top_role and ctx.author.id != ctx.guild.owner.id:
       nope = discord.Embed(color=Colors.yellow, description=f"{Emojis.Emojis.warninging} {ctx.author.mention}: you can't jail {member.mention}")
       await sendmsg(self, ctx, None, nope, None, None, None) 
       return
     ...
     async with self.bot.db.cursor() as cursor: 
      ...
      chec = await cursor.fetchone()
      if chec is None:
        em = discord.Embed(color=Colors.yellow, description=f"{Emojis.Emojis.warninging} {ctx.author.mention} use `setme` command before using jail")
        await sendmsg(self, ctx, None, em, None, None, None) 
        return
      await cursor.execute("SELECT * FROM jail WHERE guild_id = {} AND user_id = {}".format(ctx.guild.id, member.id))
      check = await cursor.fetchone()
      if check is None:
        em = discord.Embed(color=Colors.yellow, description=f"{Emojis.Emojis.warninging} {ctx.author.mention}: {member.mention} is not jailed")
        await sendmsg(self, ctx, None, embed, None, None, None) 
        return
      ...
      await self.bot.db.commit()
      embed = discord.Embed(color=Colors.green, description=f"{Emojis.check} {ctx.author.mention}: {member.mention} unjailed")             
      await sendmsg(self, ctx, None, embed, None, None, None) 
