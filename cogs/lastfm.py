import discord, button_paginator as pg, json, aiohttp, utils.userhandler as userhandler, utils.lastfmhandler as lastfmhandler
          ...
          if check[1] == message.content:
            ...
            async with message.channel.typing():
              ...
              if res is not None:   
                ...
                embed = discord.Embed(description=f"<:lastfm:1038127886797918269> **{member}** doesn't have a **Last.fm account** linked. Use `;lf set <username>` to link your **account**.", color=Colors.red)
                await message.reply(embed=embed, mention_author=False)  
      except Exception as e:
          embed = discord.Embed(description=f"{Emojis.wrong} {message.author.mention}: unable to get **{member.name}'s** recent track - {e}", color=Colors.red)
...
through the bot!""",color = 0x2F3136)
...
>   the other commands are displayed in the help command""", inline = False)
    ...
    async def set(self, ctx, ref=None):
            ...
            if not await userhandler.lastfm_user_exists(ref):
                embed = discord.Embed(description=f' **Invalid** Last.fm username',
                                    color=Colors.red)
                return await sendmsg(self, ctx, None, embed, None, None, None)
            else:
                async with self.bot.db.cursor() as cursor:
                    ...
                    embed = discord.Embed(description=f' {ctx.message.author.mention}: Your **Last.fm** username has been set to **{ref}**', color=Colors.red)
    ...
    async def remove(self, ctx): 
     async with self.bot.db.cursor() as cursor:
        await cursor.execute("SELECT * FROM lastfm WHERE user_id = {}".format(ctx.author.id))
        check = await cursor.fetchone()
        if check is None: return await sendmsg(self, ctx, None, discord.Embed(description=f" You don't have a **Last.fm account** linked. Use `;lf set <username>` to link your **account**.", color=Colors.red), None, None, None)             
        await cursor.execute("DELETE FROM lastfm WHERE user_id = {}".format(ctx.author.id))
        await self.bot.db.commit()
        await sendmsg(self, ctx, None, discord.Embed(color=Colors.green, description=f"{Emojis.check} {ctx.author.mention}: your last fm account has been removed"), None, None, None)
    ...
    async def customcommand(self, ctx, *, cmd=None):
        ...
        if cmd == "none":   
             async with self.bot.db.cursor() as cursor:
                    ...
                    embed = discord.Embed(description=f' {ctx.message.author.mention}: Your **Last.fm** custom command got deleted', color=Colors.red)
                    await sendmsg(self, ctx, None, embed, None, None, None)   
                    await self.bot.db.commit()  
                    return          
        async with self.bot.db.cursor() as cursor:
                    ...
                    embed = discord.Embed(description=f'<:lastfm:1038127886797918269> {ctx.message.author.mention}: Your **Last.fm** custom command is {cmd}', color=Colors.red)
                    await sendmsg(self, ctx, None, embed, None, None, None)   
                    await self.bot.db.commit()  
...
`10` **[{topartist10}]({topartist10url})** {topartist10plays} plays""",color = 0x2F3136)
                ...
                embed = discord.Embed(description=f" You don't have a **Last.fm account** linked. Use `;lf set <username>` to link your **account**.",color=Colors.red)
                await sendmsg(self, ctx, None, embed, None, None, None)   
        except Exception as e:
            print(e)
...
`10` **[{topalbum10}]({topalbum10url})** {topalbum10plays} plays""",color = 0x2F3136)
     ...
     try:   
      ...
      async with ctx.typing():
       ...
       if len(number) > 1:
        ...
        embed = discord.Embed(description = f"{Emojis.Emojis.warninging} {ctx.message.author.mention}: `{artist}` is not a **valid** artist", color = Colors.yellow)
        await sendmsg(self, ctx, None, embed, None, None, None)   
        print(e) 
    ...
    async def nowplaying(self, ctx: commands.Context, *, member: discord.User=None):
        ...
        try:
          ...
          embed = discord.Embed(description=f"{Emojis.wrong} {ctx.author.mention}: unable to get **{member.name}'s** recent track - {e}", color=Colors.red)
          await sendmsg(self, ctx, None, embed, None, None, None)        
