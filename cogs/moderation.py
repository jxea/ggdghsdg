...
class mod(commands.Cog):
   ...
   async def restore(self, ctx: commands.Context, *, member: discord.Member=None):
    ...
    async with ctx.message.channel.typing():
     async with self.bot.db.cursor() as cursor:
      if member == ctx.author: return await ctx.reply(embed=discord.Embed(color=Colors.yellow, description="{} {}: You can't restore your roles".format(Emojis.Emojis.warninging, ctx.author.mention)))
   ...
   async def xd(self, ctx: commands.Context, *, member: discord.Member): 
    ...
    async with self.bot.db.cursor() as cursor:
      ...
      for result in check:
              ...
              if l == 10:
               messages.append(mes)
               number.append(discord.Embed(color=0x2f3136, title=f"whitelisted ({len(check)})", description=messages[i]))
               i+=1
               mes = ""
               l=0
    
    messages.append(mes)
    embed = discord.Embed(color=0x2f3136, title=f"whitelisted ({len(check)})", description=messages[i])
    number.append(embed)
    if len(number) > 1:
   ...
   async def ban(self, ctx, member: Union[discord.Member, discord.User]=None, *, reason=None):
    ...
    if isinstance(member, discord.Member):
     if member == ctx.message.author:
        e = discord.Embed(color=Colors.yellow, description=f"{Emojis.Emojis.warninging} {ctx.author.mention}: you cannot ban yourself")
        await sendmsg(self, ctx, None, e, None, None, None)
        return
    
     if member.top_role >= ctx.author.top_role and ctx.author.id != ctx.guild.owner.id:
       nope = discord.Embed(color=Colors.yellow, description=f"{Emojis.Emojis.warninging} {ctx.author.mention}: you can't ban {member.mention}")
     ...
     if ctx.guild.premium_subscriber_role in member.roles:
      ...
      async def button1_callback(interaction: discord.Interaction):
        ...
        try: 
         ...
         no = discord.Embed(color=Colors.yellow, description=f"{Emojis.Emojis.warninging} {ctx.author.mention}: i don't have enough permissions to do this")
         await interaction.response.edit_message(embed=no, ephemeral=True, view=None)
      button1.callback = button1_callback

      async def button2_callback(interaction: discord.Interaction):
        if interaction.user != ctx.author:
            em = discord.Embed(color=Colors.yellow, description=f"{Emojis.Emojis.warninging} {interaction.user.mention}: this is not your message")
            await interaction.response.send_message(embed=em, ephemeral=True)
            return
   ...
   async def unban(self, ctx, *, member: discord.User=None):
    ...
    try: 
     ...
     await sendmsg(self, ctx, None, embed, None, None, None)
       ...
       emb = discord.Embed(color=Colors.yellow, description=f"{Emojis.Emojis.warninging} {ctx.author.mention}: couldn't unban this member")
   ...
   async def kick(self, ctx: commands.Context, member: discord.Member=None, *, reason=None):
    ...
    if member == ctx.author:
     e = discord.Embed(color=Colors.yellow, description=f"{Emojis.Emojis.warninging} {ctx.author.mention}: you can't kick yourserlf")
     await sendmsg(self, ctx, None, e, None, None, None)
     return

    if member.top_role >= ctx.author.top_role and ctx.author.id != ctx.guild.owner.id:
       nope = discord.Embed(color=Colors.yellow, description=f"{Emojis.Emojis.warninging} {ctx.author.mention}: you can't kick {member.mention}")
       await sendmsg(self, ctx, None, nope, None, None, None)
       return   
   ...
   async def role(self, ctx, user : discord.Member=None, *, role : discord.Role=None):
    ...
    if role.position >= ctx.author.top_role.position and ctx.author.id != ctx.guild.owner.id: 
     e = discord.Embed(color=Colors.yellow, description=f"{Emojis.Emojis.warninging} {ctx.author.mention}: that role is above your top role")  
    ...
    if role in user.roles:
      ...
      emb = discord.Embed(color=Colors.green, description=f"{Emojis.check} {ctx.author.mention}: added {role.mention} to {user.mention}") 
      await sendmsg(self, ctx, None, emb, None, None, None)
   ...
   async def mute(self, ctx: commands.Context, member: discord.Member=None, time=None, *, reason=None):
    ...
    if member.top_role >= ctx.author.top_role and ctx.author.id != ctx.guild.owner.id:
        no = discord.Embed(color=Colors.yellow, description=f"{Emojis.Emojis.warninging} {ctx.author.mention}: you can't timeout {member.mention}")
        await sendmsg(self, ctx, None, no, None, None, None)
        return
   ...
   async def stripstaff(self, ctx: commands.Context, member: discord.Member=None):
    ...
    if member == None:
     ...
     if member.top_role >= ctx.author.top_role and ctx.author.id != ctx.guild.owner.id:
       nope = discord.Embed(color=Colors.yellow, description=f"{Emojis.Emojis.warninging} {ctx.author.mention}: you can't strip {member.mention}'s roles")
       await sendmsg(self, ctx, None, nope, None, None, None)
       return
   ...
   async def stfu(self, ctx: commands.Context, member: discord.Member = None):
    ...
    if member == None:
        ...
        embed = discord.Embed(color=Colors.yellow, description=f"{Emojis.Emojis.warninging} {ctx.author.mention}: you can't mute yourself")
        await sendmsg(self, ctx, None, embed, None, None, None)
        return
    elif member.top_role.position >= ctx.author.top_role.position and ctx.author.id != ctx.guild.owner.id:
        embed = discord.Embed(color=Colors.yellow,description=f"{Emojis.Emojis.warninging} {ctx.author.mention}: you can't mute a member with higher roles than you")
        await sendmsg(self, ctx, None, embed, None, None, None)
        return        
    else: 
     async with self.bot.db.cursor() as cursor:
      ...
      results = await cursor.fetchone()
      if results is not None:
       e = discord.Embed(color=Colors.yellow, description=f"{Emojis.Emojis.warninging} {ctx.author.mention}: this member's messages are already getting deleted")
       await sendmsg(self, ctx, None, e, None, None, None)
       return
   ...
   async def unstfu(self, ctx, member: discord.Member=None):
    ...
    if member == None:
     ...
     async with self.bot.db.cursor() as cursor:
      ...
      results = await cursor.fetchone()
      if results is None:
        em = discord.Embed(color=Colors.yellow, description=f"{Emojis.Emojis.warninging} {ctx.author.mention}: this user isn't muted")
        await sendmsg(self, ctx, None, em, None, None, None)
        return
