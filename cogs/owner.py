...
class owner(commands.Cog):
   ...
   async def unblacklist(self, ctx, *, member: discord.User=None): 
    ...
    async with self.bot.db.cursor() as cursor: 
      ...
      if check is None: return await sendmsg(self, ctx, None, discord.Embed(color=Colors.yellow, description=f"{Emojis.Emojis.warninging} {ctx.author.mention}: {member.mention} is not blacklisted"), None, None, None)
   ...
   async def blacklist(self, ctx, *, member: discord.User=None): 
    ...
    async with self.bot.db.cursor() as cursor: 
      await cursor.execute("SELECT * FROM nodata WHERE user = {}".format(member.id)) 
      check = await cursor.fetchone()
      if check is not None: return await sendmsg(self, ctx, None, discord.Embed(color=Colors.yellow, description=f"{Emojis.Emojis.warninging} {ctx.author.mention}: {member.mention} is already blacklisted"), None, None, None)
    ...
    if member == None:
     ...
     async with self.bot.db.cursor() as cursor:
      ...
      if results is not None:
       ...
       await ctx.message.add_reaction(":x_Emojis.check:")   
       await ctx.message.add_reaction(":x_sparkles:") 
       await self.bot.db.commit()
