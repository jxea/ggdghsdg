...
class Utility(Cog):
                ...
                if data:
                    for table in data:
                        ...
                        if l == 10:
                          ...
                          l=0
                    messages.append(auto)
                    embed = Embed(description = auto, color = 0x2F3136)
    ...
    async def seen(self, ctx, *, member: Member=None):
      ...
      async with self.bot.db.cursor() as cursor: 
        ...
        check = await cursor.fetchone()
        if check is None: return await sendmsg(self, ctx, None, Embed(color=Colors.yellow, description=f"{Emojis.Emojis.warninging} {ctx.author.mention}: I didn't see **{member}**"), None, None, None) 
        ts = check[2]
    ...
    async def spotify(self, ctx: Context, *, member: Member=None):
      ...
      if member.activity:
        if isinstance(member.activity, Spotify):
          ...
          await sendmsg(self, ctx, None, embed, None, None, None)
        else: return await sendmsg(self, ctx, None, Embed(color=Colors.yellow, description=f"{Emojis.Emojis.warninging} {ctx.author.mention}: This member isn't listening to spotify"), None, None, None)  
      else: return await sendmsg(self, ctx, None, Embed(color=Colors.yellow, description=f"{Emojis.Emojis.warninging} {ctx.author.mention}: This member isn't listening to spotify"), None, None, None)  
     ...
     async with self.bot.db.cursor() as cursor: 
      ...
      try:
       ...
       if len(embeds) == 1: return await ctx.reply(embed=embeds[0], mention_author=False)
        ...
        if len(check) == 0: return await sendmsg(self, ctx, None, Embed(color=Colors.yellow, description=f"{Emojis.Emojis.warninging} {ctx.author.mention}: there are no deleted messages in {ctx.channel.mention}"), None, None, None)
        await sendmsg(self, ctx, None, Embed(color=Colors.yellow, description=f"{Emojis.Emojis.warninging} {ctx.author.mention}: current snipe limit is **{len(check)}**"), None, None, None)
    ...
    async def membercount(self, ctx: Context):
      await ctx.reply(embed=Embed(color=0x2f3136, description="**{}** members".format(ctx.guild.member_count)), mention_author=False)
    ...
    async def userinfo(self, ctx: Context, *, member: Union[Member, User]=None):
     ...
     if isinstance(member, Member):
      if member.mobile_status != Status.offline:
        devices.append("<:mobile:1015320842273566720>")
      if member.web_status != Status.offline:
        devices.append("<a:globe:1015320835059368007>")
      if member.desktop_status != Status.offline:
        devices.append("<:dsk1:1015320846732116018>")
      ...
      if member.activities:
        for a in member.activities:
          if isinstance(a, Spotify):
            e.description = f"<:spotify:1022181543202017380> listening to [**{a.title}**]({a.track_url}) by [**{a.artist}**]({a.track_url}) on spotify"
    ...
    async def server(self, ctx: Context, choice=None, *, id: int=None):
      if choice == "info" or choice is None:
        ...
        if guild is None:
            e = Embed(color=Colors.yellow, description=f"{Emojis.Emojis.warninging} {ctx.author.mention}: unable to find this guild")
            await sendmsg(self, ctx, None, e, None, None, None)
            return 
